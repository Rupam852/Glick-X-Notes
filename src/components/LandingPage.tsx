...
export default function LandingPage({ onLogin, onSignup }: LandingPageProps) {
  ...
  const [mockupActiveView, setMockupActiveView] = useState<'dashboard' | 'settings' | 'editor'>('dashboard');
  const [mockupEditingNote, setMockupEditingNote] = useState<any>(null);
  const [mockupSaving, setMockupSaving] = useState(false);
  const [mockupSavedTime, setMockupSavedTime] = useState<string | null>(null);
  ...
  const [simulatedNotes, setSimulatedNotes] = useState([
    {
      id: 1,
      title: "fhc",
      body: "Drafting the security schemas for the base64 multi-sync database module. All credentials and configuration keys are cryptographically isolated on client side.",
      color: "#6366f1",
      tags: ["security", "sync", "v2"],
      attachmentCount: 1,
      attachments: [
        { name: "schema.json", size: 1024, type: "application/json" }
      ],
      date: "APR 8, 2026",
      time: "10:52"
    },
    ...
    },
    {
      id: 3,
      title: "fnfb",
      body: "fhdnfc: local caching database initialized successfully without composite index overhead.",
      color: "#10b981",
      tags: ["database", "local-first"],
      attachmentCount: 2,
      attachments: [
        { name: "local_cache.db", size: 12048, type: "application/octet-stream" },
        { name: "benchmarks.txt", size: 4096, type: "text/plain" }
      ],
      date: "APR 8, 2026",
      time: "09:40"
    },
    {
      id: 4,
      title: "qq",
      body: "Drafting critical recovery passwords for decentralized offline access rules. Keep this safe.",
      color: "#f59e0b",
      tags: ["recovery", "passwords"],
      attachmentCount: 0,
      attachments: [],
      date: "APR 8, 2026",
      time: "09:40"
  ...
  const filteredMockupNotes = simulatedNotes.filter(note => 
    ...
    note.tags.some(tag => tag.toLowerCase().includes(mockupSearch.toLowerCase()))
  );
        ...
        >
          ...
          <div className={`grid grid-cols-1 md:grid-cols-4 min-h-[500px] text-left transition-colors duration-300 ${mockupTheme === 'dark' ? 'bg-slate-950 text-white' : 'bg-white text-slate-900'}`}>
            ...
            <div className={`md:col-span-3 p-6 md:p-8 space-y-6 flex flex-col justify-between transition-colors ${mockupTheme === 'dark' ? 'bg-slate-900/20' : 'bg-slate-100/30'}`}>
                ...
                <div className="space-y-6 flex-1">
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">{simulatedNotes.length} notes • 0.03 KB used</p>
                    </div>
                    <button 
                      onClick={() => {
                        const newNote = {
                        ...
                        setMockupEditingNote(newNote);
                        setMockupActiveView('editor');
                        setMockupSavedTime(null);
                      }}
                      ...
                      {filteredMockupNotes.length > 0 ? (
                        filteredMockupNotes.map(note => (
                          ...
                          >
                            ...
                            <div>
                              <div className="flex items-center justify-between gap-1">
                                <h4 className={`font-bold text-sm truncate transition-colors ${mockupTheme === 'dark' ? 'text-slate-100' : 'text-slate-800'}`}>{note.title || "Untitled Note"}</h4>
                                {note.attachmentCount > 0 && <span className="text-[10px] text-slate-400 font-semibold shrink-0">📎 {note.attachmentCount}</span>}
                              </div>
                              ...
                              <div className="flex flex-wrap gap-1 mt-2">
                                {note.tags.map(t => (
                                  <span key={t} className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase ${mockupTheme === 'dark' ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-550'}`}>{t}</span>
                                ))}
                              </div>
                            </div>
              ...
              ) : mockupActiveView === 'editor' && mockupEditingNote ? (
                /* Interactive NoteEditor Mockup */
                <div className="space-y-4 flex-1 flex flex-col">
                  {/* Editor Header */}
                  <div className="flex items-center justify-between pb-3 border-b border-slate-800/10 dark:border-slate-800/60">
                    <div className="flex items-center gap-2">
                      <button 
                        onClick={() => setMockupActiveView('dashboard')}
                        className={`p-1.5 rounded-lg transition-colors cursor-pointer ${mockupTheme === 'dark' ? 'hover:bg-slate-850 text-slate-400' : 'hover:bg-slate-200 text-slate-600'}`}
                      >
                        <ArrowLeft className="w-4 h-4" />
                      </button>
                      <div className="text-[9px] font-bold uppercase tracking-widest text-slate-400">
                        {mockupSaving ? (
                          <span className="flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin text-indigo-500" /> Saving...</span>
                        ) : mockupSavedTime ? (
                          <span className="text-emerald-500">Saved {mockupSavedTime}</span>
                        ) : "Draft Mode"}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <button 
                        onClick={() => {
                          ...
                          setTimeout(() => {
                            ...
                            setMockupSavedTime(now);
                            // Save note in simulatedNotes list
                            setSimulatedNotes(prev => {
                              const exists = prev.find(n => n.id === mockupEditingNote.id);
                              if (exists) {
                                return prev.map(n => n.id === mockupEditingNote.id ? { ...mockupEditingNote } : n);
                              } else {
                                return [{ ...mockupEditingNote, id: Date.now(), date: "TODAY" }, ...prev];
                              }
                            });
                          }, 800);
                        }}
                        className="bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1.5 transition-all cursor-pointer shadow-lg shadow-indigo-500/20"
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 flex-1">
                    {/* Left Pane (Metadata) */}
                    <div className="md:col-span-1 space-y-4 pr-0 md:pr-4 md:border-r border-slate-800/10 dark:border-slate-800/60 text-left">
                      {/* Title */}
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-bold text-slate-450 uppercase tracking-wider">Title</label>
                        <input 
                          type="text"
                          value={mockupEditingNote.title}
                          onChange={(e) => setMockupEditingNote({...mockupEditingNote, title: e.target.value})}
                          placeholder="Untitled Note"
                          className={`w-full px-2.5 py-1.5 text-xs font-bold border rounded-lg outline-none focus:ring-1 focus:ring-indigo-500 transition-all ${mockupTheme === 'dark' ? 'bg-slate-900 border-slate-800 text-white placeholder-slate-700' : 'bg-white border-slate-200 text-slate-800 placeholder-slate-350'}`}
                        />
                      </div>

                      {/* Color Picker */}
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-bold text-slate-450 uppercase tracking-wider">Theme Color</label>
                        <div className="flex gap-1.5 flex-wrap">
                          {['#6366f1', '#f43f5e', '#f59e0b', '#10b981', '#0ea5e9', '#8b5cf6', '#64748b'].map(c => (
                            <button
                              key={c}
                              onClick={() => setMockupEditingNote({...mockupEditingNote, color: c})}
                              className={`w-4 h-4 rounded-full border cursor-pointer transition-all ${mockupEditingNote.color === c ? 'border-white scale-125 ring-1 ring-offset-2 ring-indigo-500' : 'border-transparent hover:scale-110'}`}
                      ...
                      {/* Tags */}
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-bold text-slate-450 uppercase tracking-wider">Tags</label>
                        <div className={`flex items-center gap-1.5 border rounded-lg px-2.5 py-1.5 ${mockupTheme === 'dark' ? 'bg-slate-900 border-slate-800 text-slate-300' : 'bg-white border-slate-200 text-slate-600'}`}>
                          <Tag className="w-3 h-3 text-slate-400 shrink-0" />
                          <input 
                            type="text"
                            value={mockupEditingNote.tags.join(', ')}
                            onChange={(e) => setMockupEditingNote({...mockupEditingNote, tags: e.target.value.split(',').map(t => t.trim()).filter(Boolean)})}
                      ...
                      {/* Files */}
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-bold text-slate-450 uppercase tracking-wider">Attachments</label>
                        <div className="space-y-1 max-h-[80px] overflow-y-auto pr-1">
                          {mockupEditingNote.attachments && mockupEditingNote.attachments.length > 0 ? (
                            mockupEditingNote.attachments.map((file: any) => (
                              <div key={file.name} className={`flex items-center justify-between px-2 py-1 rounded border text-[9px] ${mockupTheme === 'dark' ? 'bg-slate-900/80 border-slate-800' : 'bg-slate-50 border-slate-150'}`}>
                            ...
                            <p className="text-[8px] text-slate-400 italic">No files attached</p>
                          )}
                        </div>
                      </div>
                    </div>

                    {/* Right Pane (Rich Editor textarea) */}
                    <div className="md:col-span-2 space-y-3 pl-0 md:pl-4 flex flex-col">
                      {/* Formatting Toolbar */}
                      <div className={`flex items-center gap-2 p-1.5 rounded-lg border text-slate-405 ${mockupTheme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-150'}`}>
                        <button className="hover:text-indigo-500 cursor-pointer font-bold text-xs p-1">B</button>
                        <button className="hover:text-indigo-500 cursor-pointer italic text-xs p-1">I</button>
                        <button className="hover:text-indigo-500 cursor-pointer font-bold text-xs p-1">H1</button>
                        <button className="hover:text-indigo-500 cursor-pointer text-xs p-1">“</button>
                        <button className="hover:text-indigo-500 cursor-pointer text-xs p-1">List</button>
                        <button className="hover:text-indigo-500 cursor-pointer text-xs p-1">Link</button>
                      </div>

                      {/* Note Body Textarea */}
                      <textarea 
                        value={mockupEditingNote.body}
                        onChange={(e) => setMockupEditingNote({...mockupEditingNote, body: e.target.value})}
                        placeholder="Start typing note content here..."
                        className={`w-full flex-1 min-h-[160px] p-3 border rounded-xl outline-none focus:ring-1 focus:ring-indigo-500 transition-all font-sans resize-none text-[11px] leading-relaxed ${mockupTheme === 'dark' ? 'bg-transparent border-slate-850 text-white placeholder-slate-700' : 'bg-transparent border-slate-150 text-slate-850 placeholder-slate-350'}`}
                      />
                    </div>
                  </div>
                </div>
